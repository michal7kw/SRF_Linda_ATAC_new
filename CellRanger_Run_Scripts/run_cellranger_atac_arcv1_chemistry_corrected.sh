...
OUTPUT_DIR="/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda/ATAC_data/cellranger_atac_arcv1_output_fixed2"
TMP_DIR="/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda/ATAC_data/tmp_fixed2"
...
BARCODE_WHITELIST_RC_FILE="atac_737K-arc-v1_rc.txt"
BARCODE_WHITELIST="$BARCODE_WHITELIST_DIR/$BARCODE_WHITELIST_FILE"
BARCODE_WHITELIST_RC="$BARCODE_WHITELIST_DIR/$BARCODE_WHITELIST_RC_FILE"
...
if [[ ! -f "$BARCODE_WHITELIST_RC" ]]; then
    echo "Creating reverse complement of ATAC barcode whitelist..."
    cat "$BARCODE_WHITELIST" | rev | tr 'ACGT' 'TGCA' > "$BARCODE_WHITELIST_RC"
    echo "Reverse complement whitelist created at $BARCODE_WHITELIST_RC"
else
    echo "Reverse complement barcode whitelist already exists: $BARCODE_WHITELIST_RC"
fi
