#!/bin/bash
#SBATCH --job-name=bulk_atac_seq
#SBATCH --output=logs/bulk_atac_seq_%A_%a.out
#SBATCH --error=logs/bulk_atac_seq_%A_%a.err
#SBATCH --array=0-1
...
# Bulk ATAC-seq Processing Pipeline
...
OUTPUT_DIR="/beegfs/scratch/ric.sessa/kubacki.michal/SRF_Linda/ATAC_data/bulk_atac_output"
...
echo "=========================================="
echo "Processing bulk ATAC-seq for sample: $SAMPLE"
echo "Date: $(date)"
echo "=========================================="
...
echo "  R1 (50bp genomic): $R1_FILE"
echo "  R3 (49bp genomic): $R3_FILE"
...
# Choose alignment strategy based on available tools
if [[ -n "$BWA" ]]; then
    ...
    # Create Bowtie2 index if it doesn't exist
    BOWTIE2_INDEX_PREFIX="$TMP_DIR/bowtie2_index/genome"
    if [[ ! -f "${BOWTIE2_INDEX_PREFIX}.1.bt2" ]]; then
        echo "Creating Bowtie2 index (this may take 10-20 minutes)..."
        mkdir -p "$TMP_DIR/bowtie2_index"
        $BOWTIE2-build --threads $SLURM_CPUS_PER_TASK "$REFERENCE_FASTA" "$BOWTIE2_INDEX_PREFIX"
    ...
    $BOWTIE2 -x "$BOWTIE2_INDEX_PREFIX" \
        -1 "$TRIMMED_DIR/${SAMPLE}_R1_paired.fq.gz" \
        -2 "$TRIMMED_DIR/${SAMPLE}_R3_paired.fq.gz" \
    ...
    echo "ERROR: No suitable aligner found!"
    echo ""
    echo "Please install tools first by running:"
    echo "  bash setup_conda_tools.sh"
    echo ""
    echo "Or install manually:"
    echo "  conda install -c bioconda bwa samtools macs2"
    exit 1
fi
...
cat > "$REPORT_FILE" << EOF
Bulk ATAC-seq Processing Report
================================
Sample: $SAMPLE
...
echo "=========================================="
echo "Bulk ATAC-seq processing completed for $SAMPLE"
...
echo "Processing pipeline completed for $SAMPLE"